...
LogicalSystem::LogicalSystem() {
  ...
  reg_post("/user_login", [](std::shared_ptr<HttpConnection> connection) {
    ...
    auto name = src_root["user"].asString();
    auto pwd = src_root["passwd"].asString();
    user_info users{0,email,pwd};
    auto reuslt =
        MySqlMgr::getInstance()->LoginUser(email, pwd,users); // 调用MySqlMgr登录用户
    ...
    auto stub_m=std::move(gRPCServerPool::getInstance()->get_stub());//获取gRPC链接
    ...
    grpc::Status Status_m= stub_m->GetChatServer(&client_context,grpc_request,&grpc_respon);
    if(Status_m.ok()){
    root["error"] =ErrorCodes::Success; // 设置成功码
    root["uid"]=users.uid;
    ...
    gRPCServerPool::getInstance()->release_stub(std::move(stub_m));//归还gRPC链接
  ...
  reg_post("/get_varify_code_forget_passwd", [](std::shared_ptr<HttpConnection>//获取忘记密码验证码服务
                                                    connection) {
    ...
    if (!parse_success) {
      ...
      return true;
        ...
        bool select_result=MySqlConn->ExistsUser(src_root["email"].asString(),
        ...
        LogQueue::getInstance()->push(std::make_pair<LogLevel,std::string>(
            LogLevel::INFO, "Processing verification code request for email: " + src_root["email"].asString()));
        GetVarifyRsp rsp = VarfiyGrpcClient::getInstance()->GetVarifyCode(
            src_root["email"].asString()); // 调用gRPC客户端获取验证码
        if (rsp.error() == ErrorCodes::Success) {
            LogQueue::getInstance()->push(std::make_pair<LogLevel,std::string>(
                LogLevel::INFO, "Verification code sent successfully to email: " + src_root["email"].asString()));
            root["error"] = ErrorCodes::Success; // 设置成功码
            root["email"] = src_root["email"];
            ...
            LogQueue::getInstance()->push(std::make_pair<LogLevel,std::string>(
                LogLevel::ERROR, std::string("Failed to send verification code to email: ") 
                + src_root["email"].asString() + std::string(", error code: ") + std::to_string(rsp.error())));
  ...
  reg_post("/submit_forget_passwd",[](std::shared_ptr<HttpConnection>connection){// 忘记密码提交服务
    auto body_str = beast::buffers_to_string(connection->request_.body().data());
    std::cout << "Received POST request with body: " << body_str << std::endl;
    Json::Value root;
    Json::Reader reader;
    ...
    if (b_get_varfiy_code->empty()) {// 验证码不存在或过期
      LogQueue::getInstance()->push(std::make_pair<LogLevel, std::string>(
          LogLevel::ERROR, "The varify code is not exist in redis or expired!"));
      root["error"] = ErrorCodes::VarifyEXpired; // 设置错误码
      std::string jsonstr = root.toStyledString();
      std::cout<<jsonstr<<std::endl;
      // 输出错误信息
    ...
    int64_t result = MySqlConn->Change_Passwd_When_User_Forget_Passwd(
        ...
        {
            LogQueue::getInstance()->push(std::make_pair<LogLevel,std::string>(
                LogLevel::ERROR, "Failed to change password for user: " + src_root["email"].asString()));
            root["error"]=ErrorCodes::MySqlError; // 设置错误码
            std::string jsonstr = root.toStyledString();
            redispool->returnConnection(redis_conn); // 归还Redis连接
            beast::ostream(connection->response_.body())<<jsonstr; // 输出错误信息
        ...
        {
            LogQueue::getInstance()->push(std::make_pair<LogLevel,std::string>(
                LogLevel::ERROR, "Email is not match: " + src_root["email"].asString()));
        ...
        {
            LogQueue::getInstance()->push(std::make_pair<LogLevel,std::string>(
                LogLevel::INFO, "Password changed successfully for user: " + src_root["email"].asString()));
            root["error"] = ErrorCodes::Success; // 设置成功码
            root["email"] = src_root["email"];
            root["uid"] = src_root["uid"];
            std::string jsonstr = root.toStyledString();
