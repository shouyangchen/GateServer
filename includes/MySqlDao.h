...
public:
    ...
    int64_t LoginUser(const std::string& name, const std::string& pwd,user_info &user);
...
inline int64_t  MysqlDao::LoginUser(const std::string& email, const std::string& pwd,user_info &user)
{
    try{
        ...
        if(!conn)
        {
            ...
            <<mysqlpp::quote<<email<<","
            <<mysqlpp::quote<<pwd<<","
            <<"@result,"<<"@USER_UID);"; // 使用 @result 变量来获取存储过程的返回�?
            std::cout<<"Login_user query: "<<query.str()<<std::endl;
            if(!query.execute())
            {
                LogQueue::getInstance()->push(std::make_pair(LogLevel::ERROR, "Failed to login user: " + email));
                pool_->releaseConnection(conn); // 释放连接
                return ErrorCodes::MySqlError;
            }
            // 获取 @result 的�?
